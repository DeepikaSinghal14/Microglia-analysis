...
# 6. Convert the correlation matrix to absolute values
abs_cor_matrix <- abs(cor_matrix)

# 7. Calculate the mean absolute correlation for each feature
...
# 8. Print the mean absolute correlation for each feature
...
# Step 5: Filter out outliers (but keep medoids)
filtered_df <- umap_df %>%
  filter(!(cluster_label == "Surveillant" & (UMAP2 > 10 | UMAP2 < -2))) %>%
  filter(!(cluster_label == "Primed" & (UMAP2 < -10 | UMAP1 < -5)))

# Step 5.1: Identify the cells that were removed from Surveillant and Primed clusters
...
# Step 5.2: Combine removed cells into one data frame for easier viewing
removed_cells <- bind_rows(removed_surv_cells, removed_primed_cells)

# Step 5.3: View removed cells (Surveillant and Primed)
print("Removed cells (Surveillant and Primed clusters):")
print(removed_cells)

# Step 1: Identify removed cells (as already done above)
removed_cells <- bind_rows(removed_surv_cells, removed_primed_cells)

# Step 2: Merge the removed cells with the original features in df
removed_features <- inner_join(removed_cells, df, by = "cell_id")

# Step 3: View the features of the removed cells
print("Features of removed cells:")
head(removed_features)  # Display first few rows to inspect



# Step 1: Filter out the cells that were included in the final analysis (i.e., not removed)
included_df <- umap_df %>%
  filter(!(cluster_label == "Surveillant" & (UMAP2 > 10 | UMAP2 < -2))) %>%
  filter(!(cluster_label == "Primed" & (UMAP2 < -10 | UMAP1 < -5)))

# Step 2: Merge the included cells with the original features in df
final_included_df <- inner_join(included_df, df, by = "cell_id")

# Step 3: View the final data frame with included cells
